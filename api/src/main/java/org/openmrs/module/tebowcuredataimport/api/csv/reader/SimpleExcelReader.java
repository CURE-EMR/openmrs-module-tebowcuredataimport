...
public class SimpleExcelReader {
    ...
    public void addPersonAttributes(String fileName, PersonAttributeType personAttributeType) throws IOException {
        ...
        log.error(count + "imported successfully");
        workbook.close();
        inputStream.close();
    }
    ...
    public void addTypedAddress() throws IOException {
        String excelFilePath = "/opt/openmrs/modules/typedAddress.xlsx";
        FileInputStream inputStream = new FileInputStream(new File(excelFilePath));
        ...
        while (iterator.hasNext()) {
            try {
                ...
                Cell c = nextRow.getCell(1);
                c.setCellType(Cell.CELL_TYPE_STRING);
                String addressValue = c.getStringCellValue();

                if (addressValue != null && addressValue.length() > 0 && !addressValue.equalsIgnoreCase("NULL")) {
                    for (PatientIdentifier id : oldIdentifiers) {
                        if (id.getIdentifier().equalsIgnoreCase(patientUUID)) {
                            PersonAddress personAddress = id.getPatient().getPerson().getPersonAddress();
                            personAddress.setAddress5(addressValue);
    ...
    public void addMiddleName() throws IOException {
        String excelFilePath = "/opt/openmrs/modules/middleName.xlsx";
        FileInputStream inputStream = new FileInputStream(new File(excelFilePath));
        ...
        while (iterator.hasNext()) {
            try {
                ...
                if (addressValue != null && addressValue.length() > 0 && !addressValue.equalsIgnoreCase("NULL")) {
                    for (PatientIdentifier id : oldIdentifiers) {
                        if (id.getIdentifier().equalsIgnoreCase(patientUUID)) {
                            PersonName personName = id.getPatient().getPerson().getPersonName();
                            personName.setMiddleName(addressValue);
