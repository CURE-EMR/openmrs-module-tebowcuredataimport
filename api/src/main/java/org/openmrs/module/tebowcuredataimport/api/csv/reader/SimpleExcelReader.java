...
public class SimpleExcelReader {
	...
	public void preparePersonAttributesImport() throws APIException, IOException {
		String mapping = Context.getAdministrationService()
				.getGlobalProperty("tebowcuredataimport.fileNamePersonAttributeMap");
		...
		for (String s : values) {
			if (s.indexOf(":") > 0) {
				...
				PersonAttributeType personAttributeType = Context.getPersonService()
						.getPersonAttributeTypeByName(s.substring(s.indexOf(":") + 1, s.length()));
	...
	public void addDateofBirth() throws IOException {
		...
		PatientIdentifierType it = Context.getPatientService()
		...
		List<Patient> patients = Context.getPatientService().getAllPatients();// Do
																				// we
																				// really
																				// have
																				// to
	...
	public void addPersonAttributes(String fileName, PersonAttributeType personAttributeType) throws IOException {
		...
		List<PatientIdentifier> identifiers = Context.getPatientService().getPatientIdentifiers(null, oldIds, null,
				null, null); // Do we really have to load all patients!!
	...
	public void addIds() throws IOException {
		...
		PatientIdentifierType patientIdentifier = Context.getPatientService()
				.getPatientIdentifierTypeByUuid("81433852-3f10-11e4-adec-0800271c1b75");
		List<PatientIdentifier> oldIdentifiers = Context.getPatientService().getPatientIdentifiers(null, oldIds, null,
				null, null); // Do we really have to load all patients!!
		while (iterator.hasNext()) {
			try {
				Row nextRow = iterator.next();
	...
	public void saveAllPatients() {
		PatientService ps = Context.getPatientService();
		PatientIdentifierType oldIdentifierType = ps
				.getPatientIdentifierTypeByUuid("8d79403a-c2cc-11de-8d13-0010c6dffd0f");
		PatientIdentifierType cchuIdentifierType = ps
				.getPatientIdentifierTypeByUuid("81433852-3f10-11e4-adec-0800271c1b75");
